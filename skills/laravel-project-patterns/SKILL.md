...
description: "Apply the catalog's Laravel controller create/destroy-test conventions through selective local retrieval when project guidance or the task calls for these patterns."
---

# Laravel Project Patterns

Covers Pest HTTP GET `create` and DELETE `destroy` tests, including reactivation
through a deactivation subresource. For other behavior, report the coverage gap and follow project conventions without searching this catalog.
...
or [destroy](references/tests/controllers/12-destroy-test-order.md). A specialized
example covers its stated assertions; retrieve another only for missing contracts. Search uncovered requirements with relevant
